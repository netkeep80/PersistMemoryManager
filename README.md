...
- **Персистентные типы** — встроенные `pstring` (мутабельные строки), `pstringview` (интернированные строки), `pmap<K,V>` (AVL-словарь), `pvector<T>` (вектор)
...
│  pptr<T> / pstring / pstringview / pmap<K,V> / pvector<T> │
├─────────────────────────────────────────────────────────┤
│             AllocatorPolicy                              │
