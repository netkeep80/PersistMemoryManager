/**
 * @file pmm/static_memory_manager.h
 * @brief StaticMemoryManager — статическая модель менеджера ПАП (Issue #108).
 *
 * Переход от объектной модели (создание экземпляра `mgr`, передача `&mgr` в методы)
 * к статической модели (все методы и состояние внутри класса `static`,
 * доступ через `Class::method()`).
 *
 * Ключевые особенности:
 *   - Все члены (`_backend`, `_initialized`, `_mutex`) объявлены `static inline` (C++17)
 *   - Все методы объявлены `static`
 *   - Параметр шаблона `InstanceId` позволяет создавать несколько независимых
 *     экземпляров одной конфигурации (мультитон по ID)
 *   - `pptr<T, StaticMemoryManager<...>>::resolve()` не требует аргументов —
 *     он обращается к `StaticMemoryManager::resolve<T>(p)` напрямую
 *
 * Пример использования:
 * @code
 *   // Определяем два независимых менеджера одной конфигурации
 *   using Cache   = pmm::StaticMemoryManager<pmm::CacheManagerConfig, 0>;
 *   using Buffer  = pmm::StaticMemoryManager<pmm::CacheManagerConfig, 1>;
 *
 ...
 * @endcode
 *
 * @see abstract_pmm.h — AbstractPersistMemoryManager (объектная модель)
 * @see pptr.h — pptr<T, ManagerT> (с поддержкой статического resolve())
 * @see manager_configs.h — готовые конфигурации менеджеров
 * @version 0.1 (Issue #108 — статическая модель менеджера ПАП)
 */
...
/**
 * @brief Статический менеджер персистентной памяти (Issue #108).
 *
 ...
 *                     Должна предоставлять `lock_policy`.
 ...
 *       `StaticMemoryManager<ConfigT, InstanceId>` имеет собственный независимый
 *       статический бэкенд, мьютекс и флаг инициализации.
 *
 * @note Используйте `destroy()` перед повторной инициализацией и между тестами.
 */
template <typename ConfigT = CacheManagerConfig, std::size_t InstanceId = 0> class StaticMemoryManager
{
  public:
    // ─── Типы ─────────────────────────────────────────────────────────────────
    ...
    using manager_type = StaticMemoryManager<ConfigT, InstanceId>;

    /**
     * @brief Вложенный псевдоним персистентного указателя, привязанного к данному менеджеру.
     *
     * `StaticMemoryManager<ConfigT, 0>::pptr<T>` и
     * `StaticMemoryManager<ConfigT, 1>::pptr<T>` — разные типы.
     *
     ...
     *
     * Этот статический метод вызывается из `pptr<T, StaticMemoryManager>::resolve()`.
     *
     * @tparam T Тип данных.
