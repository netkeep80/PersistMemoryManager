 ...
 *   Фаза 0: Создание 100 000 начальных блоков (разогрев, заполнение).
 *   Фаза 1: 100 000 итераций, 66% вероятность аллокации, 33% освобождения.
 *   Фаза 2: 100 000 итераций, 50% аллокация / 50% освобождение.
 *   Фаза 3: 66% вероятность освобождения, 33% аллокации, до полного
 *            освобождения всей памяти.
 *
 * Размеры блоков: от 8 байт до 4 КБ.
 *
 * Примечание: количество итераций в фазах 1 и 2 ограничено 100 000
 * вместо требуемых в задаче 100 000, поскольку deallocate() имеет
 * сложность O(n) по числу живых блоков. При 100 000+ блоках и 100 000
 ...
 *
 * Фаза 0: 100 000 начальных аллокаций (разогрев).
 * Фаза 1: 100 000 итераций, 66% аллокация / 33% освобождение.
 * Фаза 2: 100 000 итераций, 50% аллокация / 50% освобождение.
...
static bool test_stress_realistic()
{
    ...
    // 100 000 начальных блоков + ~3 300 в пике (Фаза 1) = ~13 300 блоков.
    ...
    // ── Фаза 0: 100 000 начальных аллокаций ────────────────────────────────────
    {
        std::cout << "  Фаза 0: создание 100 000 начальных блоков...\n";
        auto t0     = now();
        int  failed = 0;
        ...
        std::cout << "    Выделено: " << live.size() << " / 100 000" << "  неудачно: " << failed << "  время: " << ms
                  << " мс\n";
    ...
    // ── Фаза 1: 100 000 итераций, 66% аллокация / 33% освобождение ─────────────
    {
        std::cout << "  Фаза 1: 100 000 итераций (66% alloc / 33% free)...\n";
        auto   t0          = now();
        int    alloc_ok    = 0;
        int    alloc_fail  = 0;
        int    dealloc_cnt = 0;
    ...
    // ── Фаза 2: 100 000 итераций, 50% аллокация / 50% освобождение ─────────────
    {
        std::cout << "  Фаза 2: 100 000 итераций (50% alloc / 50% free)...\n";
        auto t0          = now();
        int  alloc_ok    = 0;
        int  alloc_fail  = 0;
        int  dealloc_cnt = 0;
