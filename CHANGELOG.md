...
### Added
- Add length-aware `pstring` assign/append and comparison semantics so embedded NUL bytes are preserved without changing the persistent string layout.
...
- `pmm/arena_internals.h` consolidates the internal arena primitives:
  ...
  - `detail::compute_growth()` and `detail::compute_growth_for_traits<AT>()` form the single growth policy honouring `grow_numerator` / `grow_denominator` / `max_memory_gb` **and** the maximum addressable arena size for the chosen `AddressTraits`. `compute_growth_for_traits<AT>()` additionally rejects any target whose total byte size cannot round-trip through `byte_off_to_idx_checked<AT>()` (i.e. would map to `AT::no_block`), preventing partially-applied growth where the backend resizes but the layout cannot index the new arena. Overflow-safe, rounded up to `granule_size` via `round_up_checked`.
...
- `validate_block_index` / `validate_block_header_full` use overflow-safe arithmetic. `is_valid_user_offset_unlocked` uses `checked_granule_offset` + `fits_range` instead of open-coded `byte_off + size`. The `validate_block_header_full` diagnostic value reported on size overflow is now a `size_t` sentinel instead of a wrapped sum.
- `BasicArenaView<AT, IsConst>` collapses the `ArenaView` / `ConstArenaView` boilerplate; `ArenaView<AT>` / `ConstArenaView<AT>` are now type aliases.
- `AllocatorPolicy::accumulate_counters` is the shared walker fold reused by `recompute_counters` and `verify_counters`, replacing two parallel hand-written walks.
...
### Changed
- Compact PMM kernel headers below the 7000-line `include/pmm/**` policy budget and align the rollout checker with that rule.
...
### Fixed
- Allow governance PRs to carry the root `.gitkeep` placeholder required by AI-agent swarm workflows.
