...
- [ ] Карта отображает реальные байты PMM: ManagerHeader синий, пустое пространство черное.
- [ ] При запуске сценария — карта обновляется в реальном времени.
- [ ] Tooltip показывает правильное смещение при наведении.
- [ ] Ползунки масштаба работают.
...
- [ ] Все 9 метрик отображаются корректно.
- [ ] Прогресс-бар used/total работает.
- [ ] Три scrolling-графика обновляются в реальном времени.
- [ ] Кнопка «Dump to stdout» вызывает dump_stats().
...
- [ ] 3 сценария запускаются и останавливаются без зависаний.
- [ ] `join()` вызывается корректно — нет SIGABRT при выходе.
- [ ] ops-счётчики увеличиваются в MetricsView.
- [ ] Карта памяти меняется при запущенных сценариях.
- [ ] Параметры сценариев доступны для редактирования через collapsible UI.
...
- [ ] Все 7 сценариев запускаются и работают стабильно.
- [ ] Сценарий Fragmentation Demo визуально показывает фрагментацию в MetricsView.
- [ ] Сценарий Large Blocks демонстрирует авторасширение (MetricsView: total_size увеличивается).
- [ ] Сценарий Persistence Cycle сохраняет и восстанавливает файл `pmm_demo.bin`.
- [ ] validate() в Persistence Cycle возвращает `true` после reload.
...
- [ ] Дерево отображает корректные значения ManagerHeader.
- [ ] Все блоки перечислены (или первые/последние 500 при > 1000).
- [ ] Клик на блок подсвечивает его в карте памяти.
- [ ] При > 1000 блоков показывается сообщение о скрытых блоках.
...
- [ ] DockSpace работает: панели можно перетаскивать.
- [ ] Help popup отображает правильную легенду.
- [ ] Settings меняет тему немедленно.
- [ ] Settings «Применить» корректно пересоздаёт PMM.
- [ ] FPS-лимит реально работает через sleep или vsync.
...
- [ ] `clang-format` — все новые файлы соответствуют `.clang-format`.
- [ ] `cppcheck` — нет предупреждений на новых файлах.
- [ ] Размер файлов ≤ 1500 строк (ограничение CI).
- [ ] `cmake --build build --target pmm_demo` — успешно на Ubuntu, macOS, Windows.
- [ ] Запуск демо под valgrind/AddressSanitizer — нет утечек и гонок данных.
